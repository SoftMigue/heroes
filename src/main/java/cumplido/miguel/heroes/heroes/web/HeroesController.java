...
public class HeroesController {
    ...
    public HeroesEntity getHeroById(@PathVariable int id) {
        return dataService.getHeroById(id);
    }

    @GetMapping(GET_HERO_BY_TEXT_URL)
    public List<HeroesEntity> getHeroesByText(@PathVariable String text)  {
        return dataService.getHeroByText(text);
    }

    @PutMapping(UPDATE_HERO_URL)
    public ResponseEntity<Void> updateHero(@PathVariable int id, @RequestBody String inputHeroName)  {
